...
**Situação:** `main` está em `v0.2.0`, bug crítico reportado em prod, precisa consertar **agora**. `develop` já tem features da próxima release (0.3.0) em andamento — você **não** quer arrastá-las pra prod.

Filosofia: **hotfix parte de `main`** (não de `develop`), corrige, volta pra `main` com tag patch, e **obrigatoriamente** faz back-merge em `develop`.
...
**Diferente do GitLab Flow**: aqui não tem cherry-pick. É merge em `main` + merge em `develop` (2 merges distintos). O fluxo é mais parecido com o de release branch — só que mais rápido e feito sob pressão.

---

## 1) Hotfix branch parte de `main`

> ⚠️ **Sempre de `main`**, nunca de `develop`. Se sair de `develop`, o fix vai junto com features incompletas que **não estão em produção** — é o cenário clássico de "fix acidentalmente trouxe feature meio pronta pra prod".
...
**Nomenclatura:** `hotfix/X.Y.Z` (a próxima patch da tag atual em main — se main está em `v0.2.0`, hotfix é `0.2.1`).
...
## 2) PR `hotfix/0.2.1 → main` — review acelerado, **merge commit**
...
Sem esse passo, o fix só existe em `main`. Na próxima release (`release/0.3.0`), o bug volta — porque `develop` não tem a correção.
...
main             ●─────────────────●───●─────── v0.2.0 ── v0.2.1
                                     │   │
                                     hotfix (PR → main + tag + back-merge pra develop)
                                                        │
release/0.3.0      ●──●──●──●──●──────────────────merge─┘
...
| production    | `main`         | **`v0.2.1`** (tag)     | ✅ (merge + tag) |
...
Conflito grande = `develop` ficou muito à frente de `main` desde a última release. Prevenção: cadência curta de releases.
